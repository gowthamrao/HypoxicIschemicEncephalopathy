...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (4203429,4218726,4086433,4141648,4223373,4080952,4062748,42742500,4082240,4013008,4329655,4021930,42538846)
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
...
LEFT JOIN
(
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (4093896,4075957)
...
from #qualified_events p
LEFT JOIN (
SELECT p.person_id, p.event_id , A.domain_concept_id
FROM #qualified_events P
JOIN (
  -- Begin Condition Occurrence Criteria
SELECT C.person_id, C.condition_occurrence_id as event_id, C.condition_start_date as start_date, COALESCE(C.condition_end_date, DATEADD(day,1,C.condition_start_date)) as end_date,
  C.visit_occurrence_id, C.condition_start_date as sort_date, C.condition_concept_id as domain_concept_id
FROM 
(
  SELECT co.* 
  FROM @cdm_database_schema.CONDITION_OCCURRENCE co
...
HAVING COUNT(DISTINCT cc.domain_concept_id) = 0
-- End Correlated Criteria
...
) A on A.person_id = P.person_id  AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= P.OP_END_DATE AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= DATEADD(day,0,P.END_DATE) AND A.END_DATE >= P.OP_END_DATE AND A.END_DATE <= P.OP_END_DATE ) cc on p.person_id = cc.person_id and p.event_id = cc.event_id
GROUP BY p.person_id, p.event_id
