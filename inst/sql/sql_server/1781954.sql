...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (45558386,37608892,1425132,4130036,45548877,4305274,4088036,35210667,4311540,4299090,4079149,45582631,4171102,4171120,4306573,1425131,45597098,372887,4244236,45601948,37092810,4290019,42496028,4016055,4016054,4014309,4014308,4014471,4014307,4016469,4016468,4014578,4014473,4014312,4014474)
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (4130036,4305274,4088036,4311540,4299090,4079149,4171102,4171120,4306573,4244236,4290019,4016055,4016054,4014309,4014308,4014471,4014307,4016469,4016468,4014578,4014473,4014312,4014474)
  and c.invalid_reason is null
...
WHERE (YEAR(E.start_date) - P.year_of_birth >= 0 and YEAR(E.start_date) - P.year_of_birth <= 1)
GROUP BY e.person_id, e.event_id
